...
\begin{enumerate}
  \item all the routers in this sub-path, except the last one, satisfy the following condition:
      ...
      \end{itemize}
  \item $LCSP(f)$ is also the sub-path of $\Omega_{f}$, where $\Omega_{f}$ is the set of contention flows with lower priority on this sub-path.
\end{enumerate}
\end{definition}

We can replace all the routers on $LCSP(f)$ by a single virtual router providing service curve $<\otimes_{R_i\in P(f)}\beta_{R_i,f}^l,\otimes_{R_i\in P(f)}\beta_{R_i,f}^u>$, and the leftover service curve calculation is simplified. For the high priority flows, the process of calculating the end-to-end equivalent service curve is also a collapsing process. But, to compute the leftover service curve at some routers, we have to know the arrival curve at this router. With the aid of LCSP, we just only to compute the arrival curve at the starting point of each sub-path. For a LCSP with $N$ routers, this method reduces $N-1$ times calculation of arrival curve and leftover service curve, which is time-efficient.
...
\begin{algorithmic}[1]
    \FOR {All flows $f_i\in F$}
        \FOR {All router $R_j\in \mathcal{R}_{f_i}$}
    ...
    \FOR {For each flow $f_i\in F$, with priority order}
        ...
        \FOR {each router $R_j\in \mathcal{R}_{f_i}$}
            \IF {$\Theta_{R_j,f_i}\neq \emptyset$}
