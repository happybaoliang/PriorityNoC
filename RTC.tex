...
\begin{algorithmic}[1]
    ...
    \FOR {each flow $f_i\in \mathcal{F}$ with priority order}
        ...
        \FOR {each router $R_j\in\mathcal{R}_i$ from destination to source}
            \STATE $\beta^{l}_{\tau_j,f_i}(t)=\beta_{\tau}^l$; $\beta_{\tau}^l=\overline{\beta^l_{R_j,f_i}\otimes\beta^{l}_{\tau}\otimes\delta_T(t)+B}$;
            \STATE $\beta^{u}_{\tau_j,f_i}(t)=\beta_{\tau}^u$; $\beta_{\tau}^u=\overline{\beta^u_{R_j,f_i}\otimes\beta^{u}_{\tau}\otimes\delta_T(t)+B}$;
        \ENDFOR
