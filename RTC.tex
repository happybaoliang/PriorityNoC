...
\begin{algorithmic}[1]
    ...
    \FOR {each flow $f_i\in \mathcal{F}$ with priority order}
        ...
        \FOR {For all the router $R_j$ in $\mathcal{R}_{f_i}$}
            ...
            \STATE $\alpha^l_{R_j,f_k}=\lfloor\min\{(\alpha^l\oslash\beta^u)\otimes\beta^l,\beta^l\}\rfloor$;
            \STATE $\alpha^u_{R_j,f_k}=\lceil\min\{(\alpha^u\otimes\beta^u)\oslash\beta^l,\beta^u\}\rceil$;
