...
\beta^{l^\prime}=(\bar{\beta}^l-\bar{\alpha}^u)\bar{\otimes}0
\end{equation}
\begin{equation}\label{betau}
\beta^{u^\prime}=\max\{(\bar{\beta}^u-\bar{\alpha}^l)\bar{\oslash}0,0\}
\end{equation}
...
\begin{figure}
  \centering
  % Requires \usepackage{graphicx}
  \includegraphics[scale=0.8]{figures/collapse.pdf}\\
  \caption{Longest collapsible sub-path of $f_2$. Since the flow control between $R_{13}$ and $R_9$ can be ignored, they are replaced by a single virtual router $R_{13,9}$.}\label{collapse}
\end{figure}
...
The Longest Collapsible Sub-Path of flow $f$, denoted by $LCSP(f)$, is the longest sub-path of $f$ satisfying the follow conditions:
\begin{enumerate}
  \item all the routers $R_i$ on this sub-path, except the last one, satisfy $\beta_{R_{13}}^l\otimes\beta_{\tau_{13},f_2}^l=\beta_{R_{13}}^l$ and $\beta_{R_{i}，f}^u\otimes\beta_{\tau_{i},f}^u=\beta_{R_{i},f}^u$.
  \item $LCSP(f)$ is also a sub-path of all the flows in $\Omega_{f}$, where $\Omega_{f}$ is the set of contention flows with lower priorities on this sub-path.
\end{enumerate}
\end{definition}

For the high priority flows, the process of calculating the end-to-end equivalent service curve is also a collapsing process. But, to compute the leftover service curve at some routers, we have to know the arrival curve at these routers. As implied previously, we can leverage the concept of LCSP and compute the arrival curve at the starting point of each LCSP instead of the router-by-router calculation, since all the routers on $LCSP(f)$ can be replaced by a single virtual router providing service curve $<\otimes_{R_i\in LCSP(f)}\beta_{R_i,f}^l,\otimes_{R_i\in LCSP(f)}\beta_{R_i,f}^u>$. For a LCSP with $N$ routers, this method reduces $N-1$ times calculation of arrival curve and leftover service curve, the leftover service curve calculation is greatly simplified, and the accuracy of performance bound is also improved due to the well-known ``Pay-Burst-Only-Once" phenomenon in DNC theory \cite{Boudec2001Network}.

\subsection{End-to-End Latency}
After obtained the traffic model, router model and flow control model, compute the end-to-end latency is still a non-trivial task. The follow four aspects should be considered carefully: (1) We should always compute the end-to-end latency by collapsing all the LCSP to a single virtual node, as the hop-by-hop computation will lead to a looser bound; (2) In the fixed-priority flit-level preemptive NoC, only the leftover service curve can be used by the low priority flows, thus, our computation must start from higher priority flows to lower priority; (3) Before computing the leftover service curve for lower priority flows, we must ensure that all the higher priority flows has been served, in our algorithm, the label ``Calculated" is used for this purpose; (4) The computed service curve for flow controller can only be applicable for specific flow, and we should compute these curves for each flows. 
...
\begin{algorithmic}[1]
    ...
    \FOR {each flow $f_i\in \mathcal{F}$ with priority order}
        \FOR {each router $R_j\in \mathcal{R}_{i}$}
            \IF {$\Theta_{R_j,f_i}\neq \emptyset$}
                \FOR {each flow $f_k\in \Theta_{R_j,f_i}$}
                    \STATE Compute $<\beta_{SA,R_j,f_k}^{l^\prime},\beta_{SA,R_j,f_k}^{u^\prime}>$;
                    \STATE Compute $<\beta_{R_j,f_k}^l,\beta_{R_j,f_k}^u>$;
                \ENDFOR
            \ENDIF
            \IF {$\beta_{SA,R_j,f_i}^l=\delta_0(t)$ and $\beta_{SA,R_j,f_i}^u=\delta_0(t)$}
                ...
                \STATE Compute $<\beta_{R_j,f_i}^l,\beta_{R_j,f_i}^u>$.
            \ENDIF
        \ENDFOR
        \FOR {each router $R_j\in\mathcal{R}_i$}
            \STATE Compute $<\beta_{\tau_j,f_i}^l,\beta_{\tau_j,f_i}^u>$.
        \ENDFOR
        ...
        \FOR {For all the router $R_j$ in $\mathcal{R}_{f_i}$}
            \IF {$\Omega_{R_j,f_i}\neq \emptyset$}
                \IF {$\forall f_i\in\Theta_{R_j,f_i}$ have label ``Calculated"}
                    \STATE Collapse all the possible $LCSP(f_i)$;
                    \STATE Compute the arrival curve $<\alpha^l_{R_j,f_i},\alpha^u_{R_j,f_i}>$;
                    \STATE Compute the arrival curve $<\alpha^l_{R_j,f_k},\alpha^u_{R_j,f_k}>$ where $f_k\in\Theta_{R_j,f_i}$;
                    \STATE Compute the aggregative arrival curve at $R_j$ of  flow $f_i$ and flows in $\Theta_{f_i,j}$;
                    \STATE Compute the leftover service curve $<\beta^{l^\prime}_{R_j,f_i},\beta^{u^\prime}_{R_j,f_i}>$ of router $R_j$ after serving $f_i$ and $\Theta_{R_j,f_i}$.
                \ENDIF
            \ENDIF
        \ENDFOR
    \ENDFOR
