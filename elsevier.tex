...
\author[label1]{Baoliang Li}
\author[label2]{Zeljko Zilic}
\author[label1]{Wenhua Dou}

\address[label1]{College of Computer Science, National University of Defense Technology, Changsha 410073, P.R. China}
\address[label2]{Department of Electrical \& Computer Engineering, McGill University, Montreal H3A-2A7, Quebec, Canada}
...
Networks-on-Chip (NoC) \sep priority-aware \sep real-time calculus \sep delay bound \sep buffer optimization
...
The network topology and flows are shown in Fig. \ref{topology}. There are four flows (i.e. $f_1$, $f_2$, $f_3$ and $f_4$) in the network, with different priority $P_4>P_1>P_2>P_3$. We perform the comparison on a set of periodical traffic, due to the restriction of LLA method \cite{73}\cite{189}. Denote by the packet length of flow $f_i$ is $F_i$ (in flits) and the injection period is $I_i$. Packets from the same flow has the same length, but packets of different flows can be of different lengths. To ease the analysis, LLA supposes the number of bits in a flit is the same as the physical channel width, and the latency of a flit traverses a router is 1 cycle. Under these assumptions, the traversing time of a packet with $F_i$ flits without contention, i.e. $L_i$, is equal to $F_i$.
