...
for i=1:1:F
    ...
    for j=size(routine,2):-1:1
        ...
        ffscl=rtcminconv(first4stageSCL,rtcminconv(scl,stagel));
        ffscu=rtcminconv(first4stageSCU,rtcminconv(scu,stageu));
        
        routine(j).ServiceCurve.SCL=ffscl;
        routine(j).ServiceCurve.SCU=ffscu;
        
        routine(j).BufSize=ComputeBufSize(ffscl,feedback,Times);
    end
    
    fc.SCL=[rtccurve([0 Inf 0])];
    fc.SCU=[rtccurve([0 Inf 0])];
    
    BufferReduction(fsc,fc,routine,feedback,flows(index(i)).Deadline,flows(index(i)).ArrivalCurve,Times);
...
function bufSize=BufferReduction(betaNI,betaFC,feedback,routine,deadline,arrivalcurve,Times)
    for i=size(routine,2):-1:1
        tempSC=rtcminconv(betaNI.SCL,betaFC.SCL);
        for j=1:1:size(routine,2)
            tempSC=rtcminconv(tempSC,routine(j).ServiceCurve.SCL);
        end
        delay=rtch(arrivalcurve,tempSC);
        while (delay<=deadline) && (routine(i).BufSize>1)
            routine(i).BufSize=routine(i).BufSize-1;
            temp.SCL=rtcminconv(routine(i).ServiceCurve.SCL,feedback);
            temp.SCU=rtcminconv(routine(i).ServiceCurve.SCU,feedback);
            tau.SCL=rtcmatminclos(temp,routine(i).BufSize,Times);
            tau.SCU=rtcmatminclos(temp,routine(i).BufSize,Times);
            if i>1
                for k=i-1:-1:1
                    routine(k).ServiceCurve.SCL=rtcminconv(first4stagel,rtcminconv(,rtcminconv(tau(1).SCL,stagel)));
                    routine(k).ServiceCurve.SCU=rtcminconv(first4stageu,rtcminconv(,rtcminconv(tau(1).SCU,stageu)));
                end
            else
                betaFC.SCL=rtcminconv(first4stagel,rtcminconv(,rtcminconv(tau(1).SCL,stagel)));
                betaFC.SCU=rtcminconv(first4stageu,rtcminconv(,rtcminconv(tau(1).SCU,stageu)));
            end
            tempSC=rtcminconv(betaNI.SCL,betaFC.SCL);
            for j=1:1:size(routine,2)
                tempSC=rtcminconv(tempSC,routine(j).ServiceCurve.SCL);
...
function SetBufferSize(index,buffersize,flowset)
    for i=1:1:size(flowset,2)
        if flowset(i).FlowId==index
            flowset(i).BufSize=buffersize;
        end
    end
end

function bufSize=ComputeBufSize(beta,tau,Times)
    buf=1;
    temp=rtcminconv(beta,tau);
    fb=rtcmatminclos(temp,buf,Times);
    while ~rtceq(rtcminconv(temp,fb(1)),beta)
        buf=buf+1;
        fb=rtcmatminclos(temp,buf,Times);
    end
    bufSize=buf;
