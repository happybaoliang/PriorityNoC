...
disp('DNC:');
DNC(length/period,length,bufSize);
end
...
tau=rtcmatminclos(beta13_f3,bufsize,times);
if rtceq(rtcminconv(beta13_f3,tau(1)),beta13_f3)
    tau=rtcmatminclos(beta9_f3,bufsize,times);
    if rtceq(rtcminconv(beta9_f3,tau(1)),beta9_f3)
        tau=rtcmatminclos(beta5_f3,bufsize,times);
        if rtceq(rtcminconv(beta5_f3,tau(1)),beta5_f3)
            tau=rtcmatminclos(beta1_f3,bufsize,times);
            if rtceq(rtcminconv(beta1_f3,tau(1)),beta1_f3)
                disp('buffer is large enough.');
                flag=true;
            end
        end
    end
...
eqscl=rtcminconv(rtcminconv(rtcminconv(rtcminconv(linkl,beta13_f3),beta9_f3),beta5_f3),beta1_f3);
fprintf('Delay Bound for Flow III:%3.3f cycles\n',rtch(acu,eqscl));
end